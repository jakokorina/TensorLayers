...
class MERASecondOnlyLayer(nn.Module):
    def __init__(self, rank: int = 2):
        ...
        self.entangler_first_layer = [
            nn.Parameter(torch.normal(mean, std, (2, 2, 2, rank), requires_grad=True)),
        ...
        self.register_parameter("disentangler_second_0", self.disentangler_second_layer)
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        dis_second = [tn.Node(self.disentangler_second_layer, "dis_second_0")]
        ...
        ent_first[0][1] ^ x_reshaped[2]
        ent_first[0][2] ^ x_reshaped[3]

        ent_first[1][0] ^ x_reshaped[4]
        ent_first[1][1] ^ x_reshaped[5]

        ent_first[2][0] ^ x_reshaped[6]
        ent_first[2][1] ^ x_reshaped[7]

        ent_first[3][0] ^ x_reshaped[8]
        ent_first[3][1] ^ x_reshaped[9]

        # Second layer disentangles
        dis_second[0][0] ^ ent_first[1][2]
        dis_second[0][1] ^ ent_first[2][2]

        # Second layer
        ent_second[0][0] ^ ent_first[0][-1]
        ent_second[0][1] ^ dis_second[0][2]

        ent_second[1][0] ^ dis_second[0][3]
        ...
        out = tn.contractors.greedy(
            [x_reshaped, *ent_first, *dis_second, *ent_second, *ent_third],
