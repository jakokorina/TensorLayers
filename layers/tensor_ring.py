...
class TRLayer(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, rank: int = 10):
        ...
        self.input_cores = [
            nn.Parameter(torch.normal(mean, std, (rank, 2, rank), requires_grad=True)) for _ in range(input_dim)
        ]
        self.output_cores = [
            nn.Parameter(torch.normal(mean, std, (rank, 2, rank), requires_grad=True)) for _ in range(output_dim)
        ]
